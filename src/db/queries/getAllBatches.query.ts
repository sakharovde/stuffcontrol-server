const getAllBatchesQuery = () => `
    with latest_sync_session as (select created_at, snapshot
         ...
         final_state AS (SELECT ab.storage_id,
                                ...
                                ab.shelf_life_days
                         FROM all_batches ab
                         GROUP BY ab.storage_id, ab.product_id, ab.batch_id, ab.product_name, ab.expiry_date,
                                  ab.manufacture_date, ab.shelf_life_days)
    select *
    from final_state
`;

export default getAllBatchesQuery;
